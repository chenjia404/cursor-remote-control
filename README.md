...
- 可按已确认项目创建定时规则：简单周期（每天 / 每周 / 每 N 小时）或 Cron。到点后自动把固定指令交给 Cursor Agent；默认每次新建任务，也可继续上次会话。上一轮未结束会跳过本轮，服务停机只补跑一次。
...
- 定时规则到期后仍走登录用户那套项目校验，只是把指令交给 Cursor Agent，不会变成任意系统命令。
